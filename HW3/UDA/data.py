...
class DATA(Dataset):
    def __init__(self, args, mode='train', dataset='m', adaptation='0'):
        ...
        del self.data_src[0]
        # print(self.data_src)
        for i in range(len(self.data_src)):
          # print(d)
            self.data_src[i] = os.path.join(self.img_dir_src, self.data_src[i])
            # print(d)

        # print(self.data_src[0:5])
        # exit()
        ...
        if self.tgt:
          ...
          del self.data_tgt[0]
          for i in range(len(self.data_tgt)):
            self.data_tgt[i] = os.path.join(self.img_dir_tgt, self.data_tgt[i])
          # self.tgt_path = os.path.join(self.img_dir_tgt, self.data_tgt)
        else:
          self.data_tgt=None
        # del self.data1[0]
    ...
    def __getitem__(self, idx):
      ...
      if self.tgt:
        ...
        ''' get data '''
        src_path=self.data_src[idx]
        tgt_path=self.data_tgt[idx]
