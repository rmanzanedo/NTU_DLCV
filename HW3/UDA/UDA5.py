...
def train_src(encoder, classifier, data_loader, tgt_encoder, critic, tgt_data_loader_eval):
    ...
    for epoch in range(args.epoch):
        ...
        ###################################
            ...
            train_info = 'Epoch: [{0}][src][{1}/{2}]'.format(epoch, idx+1, len(data_loader))
    ...
    for epoch in range(1000):
        ...
        for idx, (imgs_src, imgs_tgt, cls) in enumerate(data_loader):
            ...
            train_info = 'Epoch: [{0}][tgt][{1}/{2}]'.format(epoch, idx+1, len(data_loader))
...
def eval_src(encoder, classifier, data_loader):
    ...
    with torch.no_grad():
        for idx, (imgs_src, imgs_tgt, cls) in enumerate(data_loader):
            ...
            pred = classifier(encoder(images))
...
def eval_tgt(encoder, classifier, data_loader):
    ...
    with torch.no_grad(): # do not need to caculate information for gradient during eval
        for idx, (imgs, gt) in enumerate(data_loader):
            ...
            pred = pred.data.max(1)[1]
            # print(pred_cls.eq(labels.data).cpu().sum(), pred_cls, labels.data )
            # exit()
            # pred = torch.max(pred, dim = 1)
...
if __name__ == '__main__':
    ...
    src_encoder, src_classifier = train_src(src_encoder, src_classifier, src_data_loader, tgt_encoder, critic, tgt_data_loader_eval)
